...
function App() {
  ...
  return (
    <div className="App">
      ...
      <div className="show">{meaning ? meaning : 'Type or Click an emoji given below'}</div>
      <div className="emojis">
        <span className='emojis'>Top 25 emojis used across the world</span><br />
        {
          emojiArray.map((item) =>
            <span className='List' key={item} onClick={emojiClickHandler}>{item}</span>
          )
        }
      </div>
      <div className="footer">Source code at <a href="http://github.com/melvin175">Github</a></div>
    </div>
  );
}
